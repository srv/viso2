...
Struct SIFTmatching(vector<KeyPoint> kpts1, vector<KeyPoint> kpts2, Mat desc1, Mat desc2, bool homography) {
    ...
    if(homography == true){
        ...
        try{
            ...
            F = cv::findFundamentalMat(coord1_aft_match, coord2_aft_match, RANSACinliersMask, CV_RANSAC, 1.f) ;
...
void imageCallback(const sensor_msgs::ImageConstPtr& l_image_msg, const sensor_msgs::ImageConstPtr& r_image_msg){
    ...
    double Time_before_fast = ros::Time::now().toSec() ;
    ...
    double Time_after_fast = ros::Time::now().toSec() ;
    ...
    ROS_INFO("Time SIFT process: %f", Time_after_sift - Time_after_fast) ;
...
int main(int argc, char **argv)
{
    ...
    ROS_INFO("This is when the FAST1 begins") ;
